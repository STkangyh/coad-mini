...
| # | 항목 | 근거 |
|---|---|---|
| 1 | **base-heavy 실험을 고친 baseline으로 재실행** | ⚙️ [`run_base_heavy_split.py:124`](../dev/run_base_heavy_split.py)가 **고정** `MEM_PER_STAGE`를 쓰는데, 그 리포트 자신이 이게 GRU −65% 붕괴의 원인이라고 밝혔다. 즉 "격차가 벌어짐"의 일부는 **baseline을 굶긴 결과**일 수 있다. [`run_ucf101_gru_agem.py:112`](../dev/run_ucf101_gru_agem.py)는 이미 클래스 수 비례로 고쳐놨으므로 그 수정을 옮기면 된다 |
| 2 | **SSv2 실험에 seed 도입** | ⚙️ [`run_ssv2_temporal_pooling.py`](../dev/run_ssv2_temporal_pooling.py)에 seed/RandomState **0건** — 헤드라인 +8.97pp가 단일 실행. 06-06 리포트가 스스로 "seed 5+ 권장"이라 썼는데 미채택 |
| 3 | **pooling·구간 수 선택을 held-out으로** | ⚙️ 8종 pooling 비교와 k=1~8 sweep을 전부 val에서 하고 **그 val 수치를 보고**한다 → 24.71에 선택 편향 |
| 4 | **SSv2도 TCD 프로토콜로 평가** | ⚙️ UCF101은 남의 논문과 맞추려 TCD 프로토콜로 갔는데 SSv2는 자체 48-class/8-stage. 그러면서 ESSENTIAL 48.9와 우리 24.7을 비교 중 — **프로토콜이 다름** |
...
| 6 | **SSv2 나머지 17pp가 정말 학습형 encoder 몫인가** | 💭 24.7 vs ESSENTIAL baseline 42.1의 격차를 그렇게 귀속했으나 **우리 큐레이션 자체의 난이도**와 분리 안 됨. 개선점 4를 하면 풀린다 |
